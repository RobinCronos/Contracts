...
contract FixedStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable {
    ...
    function depositOnBehalf(address _user, uint256 _poolIndex, uint128 _amount) external onlyOwner nonReentrant whenNotPaused notContract {
        require(_poolIndex < pools.length, "Index out of bound");
        
        Pool memory _pool = pools[_poolIndex];
        UserInfo storage _userInfo = userInfo[_user][_poolIndex];
        uint32 currentDay = getCurrentDay();
        ...
        emit Deposit(_user, _poolIndex, address(_pool.token), _amount);
    }
